...
def reset_game(grid_cells, TILE):
    ...
    [cell.draw(screen, TILE) for cell in grid_cells]
    pygame.display.flip()

    # Cho phép người chơi chọn lại vị trí ban đầu
    start, end = select_start_end_manually(grid_cells, Daisize, RongSize, TILE, screen)

    for cell in grid_cells:
        cell.path = False

    return grid_cells, start, end
def xulygoiy():
...
def main():
    ...
    while True:
        ...
        if target_cell == end:
            Win_sound.play()
            game_win_text()
            #grid_cells, start, end = reset_game(grid_cells, TILE)
